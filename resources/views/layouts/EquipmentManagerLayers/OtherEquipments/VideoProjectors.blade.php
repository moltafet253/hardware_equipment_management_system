<form id="new-videoprojector">
    ...
    <div class="flex items-center">
        <div class="fixed z-10 inset-0 overflow-y-auto hidden" id="addVideoProjectorModal">
            <div
                class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center  sm:block sm:p-0">
                <div class="fixed inset-0 transition-opacity" aria-hidden="true">
                    <div class="absolute inset-0 bg-gray-500 opacity-75 addvideoprojector"></div>
                </div>
                <div
                    class="inline-block align-bottom bg-white rounded-lg text-right overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:w-full sm:max-w-[550px]">
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-headline">
                            تخصیص ویدئو پروژکتور به کاربر
                        </h3>
                        <div class="mt-4">
                            <div class="flex">
                                ...
                                <select id="videoprojector" class="border rounded-md w-96 px-3 py-2 select2" name="videoprojector">
                                    ...
                                    @php
                                        $videoprojectors = \App\Models\Catalogs\OtherEquipments\VideoProjector::join('companies', 'video_projectors.company_id', '=', 'companies.id')
                                            ...
                                            ->get([ 'companies.name', 'video_projectors.id', 'video_projectors.model']);
                                    @endphp
                                    @foreach($videoprojectors as $videoprojector)
                                        <option value="{{ $videoprojector->id }}">
                                            {{ $videoprojector->name . ' - ' . $videoprojector->model }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        ...
                        <button id="cancel-add-videoprojector" type="button"
...
<div class="flex pb-3 pt-6">
    <h3 class="font-bold pr-5 pt-2 ml-3">اطلاعات ویدئو پروژکتور</h3>
    <button type="submit"
            class="px-4 py-2 mr-3 bg-green-500 text-white rounded-md hover:bg-green-600 focus:outline-none focus:ring focus:border-blue-300 AddVideoProjector">
...
<div class="bg-white rounded shadow flex flex-col">
    <div class="max-w-full items-center overflow-x-auto">
        @php
            $eq_videoprojectors=\App\Models\EquipmentedOtherDevices\EquipmentedVideoProjector::where('person_id',$personId)->get();
        @endphp
        @if(!$eq_videoprojectors->isEmpty())
            <table class="w-full border-collapse rounded-lg overflow-hidden text-center">
                ...
                <tbody>
                @foreach($eq_videoprojectors as $videoprojector)
                    <tr class="even:bg-gray-300 odd:bg-white">
                        <td class=" px-3 py-3 "> {{ $videoprojector->property_number }}</td>
                        <td class=" px-3 py-3 ">
                            @php
                                $videoprojectorInfo = \App\Models\Catalogs\OtherEquipments\VideoProjector::join('companies', 'video_projectors.company_id', '=', 'companies.id')
                                ->select('video_projectors.*', 'companies.name as company_name')
                                ->find($videoprojector->video_projector_id);
                            @endphp
                            {{ $videoprojectorInfo->company_name  }}
                        </td>
                        <td class=" px-3 py-3 ">
        ...
        @else
            <div class="flex p-3">
                <h3 class="font-bold text-red-500 ml-4 mt-2">این کاربر ویدئو پروژکتور ثبت شده ندارد</h3>
            </div>
        @endif
    </div>
</div>
