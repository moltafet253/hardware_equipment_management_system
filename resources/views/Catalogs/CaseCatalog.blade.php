    ...
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            ...
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                <button id="new-case-button" type="button"
                ...
                <form id="new-case">
                    ...
                    <div class="mb-4 flex items-center">
                        <div class="fixed z-10 inset-0 overflow-y-auto hidden" id="newCaseModal">
                            <div
                ...
                <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                    ...
                    @foreach ($caseList as $case)
                        <tr class="bg-white">
                            ...
                            <td class="px-6 py-4">
                                @php
                                    $caseInfo=\App\Models\Catalogs\Company::find($case->company_id)
                                @endphp
                                {{ $caseInfo->name }}</td>
                            <td class="px-6 py-4">{{ $case->model }}</td>
                            <td class="px-6 py-4">
                                <button type="submit" data-id="{{ $case->id }}"
                                        class="px-4 py-2 mr-3 bg-blue-500 text-white rounded-md hover:bg-blue-600 focus:outline-none focus:ring focus:border-blue-300 CaseControl">
                                    جزئیات و ویرایش
                                </button>
                                @if($case->active==1)
                                    <button type="submit" data-id="{{ $case->id }}"
                                            class="px-4 py-2 mr-3 bg-red-500 text-white rounded-md hover:bg-red-600 focus:outline-none focus:ring focus:border-red-300 deactiveCaseControl">
                                        غیرفعالسازی
                                    </button>
                                @else
                                    <button type="submit" data-id="{{ $case->id }}"
                                            class="px-4 py-2 mr-3 bg-green-500 text-white rounded-md hover:bg-green-600 focus:outline-none focus:ring focus:border-green-300 deactiveCaseControl">
                                        فعالسازی
                                    </button>
                                @endif
                            </td>
