...
@section('content')
    <main class="flex-1 bg-cu-light py-6 px-8">
        <div class="mx-auto lg:mr-72 mb-4">
            <form id="showHistory" action="/History/getPersonalCodeHistory" method="post">
                @csrf
                <div class="flex bg-white rounded shadow px-6 py-4">
                    ...
                    <input type="text" id="personal_code" name="personal_code"
                           @if(isset($_POST['personal_code'])) value="{{ $_POST['personal_code'] }}" @endif
                    ...
                    @else
                        <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                            ...
                            @foreach($equipment_log as $logs)
                                @php
                                    if (str_contains($logs->title,'Added') || str_contains($logs->title,'from')){
                                        continue;
                                    }
                                @endphp
                                <tr class="bg-white">
