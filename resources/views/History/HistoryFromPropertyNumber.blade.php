                                ...
                                <tr class="bg-white">
                                    ...
                                    <td class="px-6 py-1">
                                        ...
                                        @php
                                            ...
                                            } elseif (str_contains($logs->title, 'Moved equipment')) {
                                                $message=json_decode($logs->title,true);
                                                $personFrom=Person::find($message['from']);
                                                $personTo=Person::find($message['to']);
                                                echo __('messages.device_moved', ['aname' => $personFrom->name, 'afamily' => $personFrom->family, 'apersonnel_code' => $personFrom->personnel_code,'bname' => $personTo->name, 'bfamily' => $personTo->family, 'bpersonnel_code' => $personTo->personnel_code]);
                                            }
                                            elseif (str_contains($logs->title, 'Changed property number')) {
                                                ...
                                                if ($nextLog) {
                                                    ...
                                                    foreach (json_decode($logs->title, true) as $value) {
                                                        switch ($value['field']) {
                                                            case 'case_id':
                                                            case 'delivery_date':
                                                                echo 'تغییر تاریخ تحویل از: ' . $value['from'] . ' به: ' . $value['to'] . '<br/>';
