                            ...
                            @foreach($equipment_log as $logs)
                                <tr class="bg-white">
                                    ...
                                    <td class="px-6 py-1">
                                        ...
                                        @endphp
                                        {{ $personInfo->personnel_code . ' - ' . $personInfo->name . ' ' . $personInfo->family }}</td>
                                    <td class="px-6 py-1">
                                        @php
                                            if (str_contains($logs->title,'Added')){
                                                 echo 'دستگاه با کد اموال '. $logs->property_number .' اضافه شد.';
                                            }
                                            elseif (str_contains($logs->title,'Assigned')){
                                                echo 'اختصاص دستگاه به ' . $personInfo->name . ' ' . $personInfo->family . ' با کد پرسنلی ' . $personInfo->personnel_code;
                                            }else{
                                        @endphp
