...
class NetworkCardController extends Controller
{
    public function newNetworkCard(Request $request)
    {
        ...
        if (!$function_type) {
            return $this->alerts(false, 'nullFunctionType', 'نوع عملکرد انتخاب نشده است');
        }
        ...
        $NetworkCard->function_type = $function_type;
        $NetworkCard->save();
    ...
    public function editNetworkCard(Request $request)
    {
        ...
        $function_type = $request->input('function_typeForEdit');
        if (!$brand) {
            return $this->alerts(false, 'nullBrand', 'نام برند انتخاب نشده است');
        }
        if (!$model) {
